...
- **Mocking 대신 in-memory 구현체 사용**: `InMemoryOrderRepository`, `InMemoryInventoryRepository`를 직접 주입해 테스트한다. (Mock 라이브러리 도입 없음)
- **Coverage 80% 목표는 본 범위에서는 강제하지 않는다.** 핵심 비즈니스 로직(Domain/Use Case) 테스트가 통과하는지에만 집중한다.
...
2. **Test Scope & Layered Strategy**:
   - **Unit Tests**: Domain Entity, Domain Policy, Use Case의 비즈니스 로직을 검증하세요.
   - **Integration Tests**: Repository, Service 등 Infrastructure 계층의 실제 동작을 검증하세요. 이 프로젝트는 in-memory 구현체를 직접 주입해 테스트합니다.
   - **E2E Tests**: 이 프로젝트 범위에서는 다루지 않습니다.

3. **Verification & Self-Correction**:
   - 구현이 완료될 때마다 전체 테스트를 실행하세요. (`npm test`)
