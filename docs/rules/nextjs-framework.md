...
- **예시**: 전체 `OrderDetailPage`가 아니라, 클릭 핸들러가 필요한 `ApproveButton`만 Client Component로 만듭니다.
...
- Server Actions는 도메인 폴더 내 `actions.ts`에 위치시킵니다 (예: `src/app/orders/actions.ts`).
